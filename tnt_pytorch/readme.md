...
```bash
python train.py data/bdd100k/images --model tnt_s_patch16_224 --sched cosine --epochs 300 --opt adamw -j 8 --warmup-lr 1e-6 --mixup .8 --cutmix 1.0 --model-ema --model-ema-decay 0.99996 --aa rand-m9-mstd0.5-inc1 --color-jitter 0.4 --warmup-epochs 5 --opt-eps 1e-8 --repeated-aug --remode pixel --reprob 0.25 --amp --lr 1e-3 --weight-decay .05 --drop 0 --drop-path .1 -b 8 --output models/
```
...
```
```bash
python train.py data/bdd100k --model tnt_s_patch16_224 -b 256 --pretrain_path models/tnt_s_81.5/ --evaluate
```
